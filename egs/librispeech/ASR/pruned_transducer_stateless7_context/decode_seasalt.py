...
from asr_datamodule import LibriSpeechAsrDataModule, SeasaltAsrDataModule
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="""Path to cuts for decoding,
        e.g. data/eval/dev_aishell/manifests/cuts_dev_aishell.jsonl.gz"""
    )
...
def decode_one_batch(
    ...
    if params.decoding_method == "fast_beam_search":
        ...
        logging.info("Running modified beam search")
...
def main():
    parser = get_parser()
    SeasaltAsrDataModule.add_arguments(parser)
    ...
    seasaltspeech = SeasaltAsrDataModule(args)
    
    dev_cuts = seasaltspeech.dev_cuts(args.cuts)
    dev_cuts = dev_cuts.map(text_normalize_for_cut)
    dev_dl = seasaltspeech.test_dataloaders(dev_cuts)
