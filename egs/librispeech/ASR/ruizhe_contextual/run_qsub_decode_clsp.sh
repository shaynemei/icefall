  ...
  for epoch in $epochs; do
    for avg in $avgs; do
      ./pruned_transducer_stateless7_context/decode.py \
          ...
          --keep-ratio 1.0 --is-full-context true --n-distractors 0
...
#   echo 
#   echo `date` "part=$part"
#   hyp_in=$exp_dir/modified_beam_search_LODR/recogs-${part}-epoch-17-avg-1-modified_beam_search_LODR-beam-size-4-rnnlm-lm-scale-0.4-LODR-2gram-scale--0.16.txt
#   hyp=$exp_dir/modified_beam_search_LODR/recogs-${part}-epoch-17-avg-1-modified_beam_search_LODR-beam-size-4-rnnlm-lm-scale-0.4-LODR-2gram-scale--0.16.hyp.txt
#   ref=data/fbai-speech/is21_deep_bias/ref/${part}.biasing_${n_distractors}.tsv
#
#   python ruizhe_contextual/recogs_to_text.py \
#     --cuts data/fbank/librispeech_cuts_${part}.jsonl.gz \
